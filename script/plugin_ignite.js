...
gxp.plugins.Ignite = Ext.extend(gxp.plugins.Tool, {
    ...
    addActions: function() {
    	...
    	OpenLayers.Control.PointLoc = OpenLayers.Class(OpenLayers.Control, {
    			...
    			draw: function () {
					this.handler = new OpenLayers.Handler.Path( control, 
						{"done": this.notice});
				},
				...
				notice: function (path) {
					...
					var userid = '0';
					startMonth = zeroPad(windData[0].data.date.getMonth() + 1,2);
					startDay = zeroPad(windData[0].data.date.getDate(),2);
					startHour = windData[0].data.time;
					startHour = startHour.replace(':','').substr(0,4);
					fuelmodel = Ext.getCmp("fuelmodel_id").getValue()
					name = Ext.getCmp("ignition_name").getValue()
					//alert(windString);
					...
					if (!fuelmodel) {
						Ext.MessageBox.show({
							msg: 'Fout: Geen terrein ID opgegeven',
							width: 300,
							wait: false
						});
					}
						...
						Ext.Ajax.request({
						   ...
						   url: '/geoserver/ows?service=WPS&request=execute&identifier=py:wildfire_startRun&datainputs=userid='+userid+';name='+name+';fuelmodel='+fuelmodel+';geom='+location+';windstring='+windString+';weatherstring='+weatherString+';startmonth='+startMonth+';startday='+startDay+';starthour='+startHour+';&RawDataOutput=string=mimeType=%22application/json%22',
						   success: wpsCallback,
						   timeout: 120000,
