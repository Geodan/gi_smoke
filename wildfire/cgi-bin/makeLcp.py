...
class makeLcp():
	...
	SELECT 1 As rid, rast FROM canvas;
		...
		print('starting gdal')
		#FUEL
		subprocess.call(self.gdal_translate_path + " -of AAIGrid -a_nodata -999 -outsize 100% 100% \"PG:host=192.168.40.5 port=3389 dbname=research user=tomt password=Gehijm schema=model_wildfire table=fuelmodel_"+str(runid)+"\" "+ self.output_path + "/fuelmodel.asc", shell=True)      
		#ELEV
		subprocess.call(self.gdal_translate_path + " -of AAIGrid -a_nodata -999 -outsize 100% 100% \"PG:host=192.168.40.5 port=3389 dbname=research user=tomt password=Gehijm schema=model_wildfire table=ahn1_"+str(runid)+"\" " + self.output_path + "/ahn1.asc", shell=True)
		#SLOPE
		subprocess.call(self.gdal_translate_path + " -of AAIGrid -a_nodata -999 -outsize 100% 100% \"PG:host=192.168.40.5 port=3389 dbname=research user=tomt password=Gehijm schema=model_wildfire table=slope_"+str(runid)+"\" " + self.output_path + "/slope.asc", shell=True)
		#ASPECT
		subprocess.call(self.gdal_translate_path + " -of AAIGrid -a_nodata -999 -outsize 100% 100% \"PG:host=192.168.40.5 port=3389 dbname=research user=tomt password=Gehijm schema=model_wildfire table=aspect_"+str(runid)+"\" " + self.output_path + "/aspect.asc", shell=True)
		#EMPTY RASTER
		subprocess.call(self.gdal_translate_path + " -of AAIGrid -a_nodata -999 -outsize 100% 100% \"PG:host=192.168.40.5 port=3389 dbname=research user=tomt password=Gehijm schema=model_wildfire table=emptyraster_"+str(runid)+"\" " + self.output_path + "/emptyraster.asc", shell=True)
		#Some unneeded steps
	...
	def start(self):
		...
		if (self.result):
			...
			self.output_path  = '/var/www/wildfire/work/fuelmodels/' + str(self.runid)
