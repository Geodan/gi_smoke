print "This module has been replace by a CGI version in /var/www/wildfire/cgi-bin/"
return
...
SELECT 
	...
	,b.run
FROM top10nl.terrein a, selectionbox b, model_wildfire.landuse2fuel c
LEFT JOIN model_wildfire.fuelmodels d ON (c.fuel_id = d.id)
...
ALTER TABLE model_wildfire.terrein_%s ADD PRIMARY KEY (gid);
	...
	conn.commit()
	#TODO: error checking
	curlstring = 'curl -v -u admin:Gehijm -XPOST -H "Content-type: text/xml" -d "<featureType><name>terrein_'+str(runid)+'</name></featureType>" http://192.168.40.8:3389/geoserver/rest/workspaces/model_wildfire/datastores/landuse/featuretypes'	
	os.system(curlstring)
	curlstring = 'curl -v -u admin:Gehijm -XPUT -H "Content-type: text/xml" -d "<featureType><nativeCRS>epsg:900913</nativeCRS></featureType>" http://192.168.40.8:3389/geoserver/rest/workspaces/model_wildfire/datastores/landuse/featuretypes/terrein_'+str(runid)
	os.system(curlstring)
	#Freakin bug, you have to add 'enabled': http://comments.gmane.org/gmane.comp.gis.geoserver.user/26753
	curlstring = 'curl -v -u admin:Gehijm -XPUT -H "Content-type: text/xml" -d "<layer><defaultStyle><name>top10nl_terrein</name></defaultStyle><enabled>true</enabled></layer>" http://192.168.40.8:3389/geoserver/rest/layers/model_wildfire:terrein_'+str(runid)
	os.system(curlstring)
